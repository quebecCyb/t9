...
corpus_root = 'ADMIN_EUR.IT/A-Administrative'
...
for i in lines:
	if i.startswith('$'):
		last_correct = i.replace('$', '')
	else:
		error_corpus.append([i, last_correct])
	
print('Готово.')

weights = a1.trainWeights(error_corpus)

assert a1.correctSpelling("retangoli leva",M2,weights,1.0) == 'rettangoli leva', "Коригираната заявка следва да е 'campionato di calcio'"
